...
if [ "${ORBIT_APP_TESTS:-0}" = "1" ] && command -v xcodebuild >/dev/null 2>&1; then
  ...
  # pipefail matters here: piping xcodebuild into grep would otherwise report
  # grep's exit code, so a failing suite would look like a passing stage. And
  # this runs BEFORE the summary below — the merge first placed it after, which
  # would have printed "all checks passed" and then failed the tests underneath
  # it. Exactly the shape FN-47 is about.
  ( set -o pipefail
    cd apps/OrbitApp \
    && xcodebuild test \
