...
17. **Origin is not residence.** `location` carries both, and they are not
    rivals: someone born in New York who lives in San Francisco has two true
    facts. Mark them so they can be told apart — a stated move or arrival gets
    `valid_from` ("in San Francisco since 2022"); a birthplace or upbringing
    ("born and raised in New York") gets **no** `valid_from` and no `valid_to`.
    Never close one with the other; the owner decides that at review.
