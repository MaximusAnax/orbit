...
(BUILD.md §1.3). **The active prompt is whichever `extraction-prompt-vN.md` is
the highest bundled** — `ExtractionPrompt.latestVersion` derives it, so adding
this file promoted it. v7's golden run is **owed, not waived**: the
`plural-attribution` golden exists and is reported as awaiting a fixture until
`swift run orbit-evals measure --live` produces one. Pin with
`ORBIT_PROMPT_VERSION=v6` to compare.

What that costs, stated plainly: every provisional PIPE number in the
ratification packet was measured against fixtures produced by v1, so those
numbers describe a prompt three bumps back. The first
`swift run orbit-evals measure --live` on a machine with a key re-measures
them. `ORBIT_PROMPT_VERSION=v1` runs the originally-measured prompt.

v7 adds rule 35 — a fact stated about a group belongs to each named member —
after a device capture recorded "both of them go to Harvard" against one of the
two people. Rule 8 had this for the speaker's own "we both…" since v1; 35 is
that same instruction with the speaker taken out of it.

The model receives: this prompt (system), the capture context
...
## Rules added in v7, from device testing

35. **A fact stated about a group belongs to each named member.** "They both go
    ...
    - **Distribute only what the group sentence actually says.** If the speaker
      ...
      fact, and it never invents a person to distribute to.
