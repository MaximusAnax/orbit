...
38. **A fact stated about a group belongs to each named member.** "They both go
    to Northeastern" is two `education` assertions, one per person, sharing the
    entity ref and the verbatim — rule 8's "we both…" with the speaker taken
    out. Only people actually named receive it, and only the fact the group
