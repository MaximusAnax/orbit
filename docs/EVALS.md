...
- **Real captures** (`mock_memos/`): the four real memos — three interaction captures plus the Eliah portrait — with confirmed transcripts, plus every future real capture Abdoul reviews. Real memos are the *validity* anchor — messy, disfluent, true.
...
- **Edit-expectation entries** *(ratified 2026-07-28, from the Eliah Google/Uber case)*: a golden may mark a source/world divergence — a spot where the speaker misspoke — and grade all three layers: the transcript stays faithful to the audio, **extraction must follow the source** (an extractor that outputs the world-truth has consulted something other than the source, which is silent correction — a P4 failure dressed as helpfulness), and the world-truth enters only as an expected human edit at review (`edited_payload`). Fidelity to the source and deference to the human are the same principle at two layers.
- **First portrait-class golden**: `docs/evals/goldens/eliah-portrait.md` (10:28 of real portrait speech; owner-reviewed ground truth, including a verified source/world divergence case).
